...
#endif
#if !HAVE_MALLOC
extern void *RkiMalloc(size_t size);
extern void *RkiCalloc(size_t num, size_t size);
# define malloc(size) RkiMalloc(size)
# define calloc(num, size) RkiCalloc(num, size)
#endif

#ifndef HAVE_MEMSET
extern void *RkiMemset(void *ptr, int ch, size_t size);
# define memset(buf, ch, size) RkiMemset(buf, ch, size)
#endif

#ifndef HAVE_STRDUP
extern char *RkiStrdup(const char *str);
