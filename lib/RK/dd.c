 ...
 * DD privates 
 */
static struct DM	*_RkCreateDM(struct DF *, unsigned char *,
				     unsigned char *, int);
static void		_RkFreeDM(struct DM *);
static struct DM	*_RkAllocDM(struct DF *, unsigned char *,
				    unsigned char *, int);
...
static struct DM *
_RkAllocDM(struct DF *df, unsigned char *dicname, unsigned char *nickname, int class)
{
...
int
DMrename(struct DM *dm, char *nickname)
{
  struct DF	*df = dm->dm_file;
  struct DDT	*ddt = dm->dm_line;
  char		*new_spec;
...
int
_RkMountMD(struct RkContext *cx, struct DM *dm, struct DM *qm, int mode, int firsttime)
{
  struct MD	*md, *head;
  struct DF	*df;
  struct DD	*dd;
