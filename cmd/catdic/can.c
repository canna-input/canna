...
int mode;
{
    ...
    fprintf(stderr,gettxt(
		  "cannacmd:256", "Cannot access to dictionary.\n"));
    return(ERR_VALUE);
}
...
char  **argv;
{
	...
	memset ((void *)init, 0, RECSZ);
	gethostname(init, RECSZ);
    }
#endif
    rk_init();
	...
	switch (ret) {
	    ...
	    break;
          case NOTUXSRV:
	    fprintf(stderr,gettxt("cannacmd:259",
		"Cannot kill server,because server is run by another machine.\n"));
	    break;
          case NOTOWNSRV:
	    fprintf(stderr,gettxt("cannacmd:260",
		"Cannot kill server,because server is not run by you.\n"));
	    break;
          default:
	    fprintf(stderr, gettxt("cannacmd:261",
		   "Invalid return code RkKillserver  code=%d \n"),ret);
	    break;
	}
	RkFinalize();
	exit(ERR_VALUE);
