...
% rcs_id[]="$Id: protocol.tex,v 1.2 2004/03/15 05:03:58 aida_s Exp $";
%
% 改版履歴
% 92/06/23      初版
% 93/02/01	ちょっとてを入れた。
...
\def\reqtypeqdic{Type 19}
\def\reqtypethrough{Type 20}
...
\def\reqQueryDictionary{\reqtypeqdic}
...
バージョン情報 $Id: protocol.tex,v 1.2 2004/03/15 05:03:58 aida_s Exp $
\end{center}
\vfill
\begin{center}
...
サーバプロセス(cannaserver)と
クライアントプロセスとのインタフェースとなるかな漢字変換プロトコルの仕様
を定義するものである。

本仕様書では、『かんな』バージョン2で追加されたワイドキャラプロトコルについてのみ記述する。
バージョン1のプロトコルは『かんな』バージョン3.7で廃止された。
...
3.3 & 3.3 & KillServerの追加 \\  \hline
3.6p4 & 同上 & QueryExtensionsの仕様を明確化、拡張 \\  \hline
\end{tabular}
...
漢字変換に関係の無いリクエストが拡張プロトコルとして定義されている。
...
\subsection{拡張プロトコル}
%
拡張プロトコルは、リクエスト名で識別され、サーバによって異なるリクエスト
番号を持つ。リクエスト番号はQueryExtensionsプロトコルによって取得される。

複数のリクエストを集めて、1つの拡張としてもよい。この場合、クライアントは、
リクエスト番号を個別に問い合わせることなく、1回のQueryExtensionsで全部の
番号を取得できる。そのかわり、サーバはこれらに一連の番号を与えなければ
ならない。

ただし、本仕様書で規定されているGetServerInfo(番号0x01)からCopyDictionary
(番号0x0a)までの拡張プロトコルは特別であり、必ずこの番号、この順序でなければ
ならない。また、0x00はリクエスト番号として使用できない。

なお、以上はCanna 3.6p4で決めた仕様であり、それまでは「QueryExtensions
リクエストには成功なら0、失敗なら$-$1を返す」という仕様であった。
成功の意味が明確でなく、またプロトコルの拡張の役に立たないため、
プロトコルを再定義した。この再定義によって互換性は保たれている。
%
\newpage
%
\subsection{共通タイプ}
...
全部で33のプロトコルに関して、リクエストとリプライそれぞれ、22、11 の
タイプに分けることができる。リクエストの22のタイプおよびリプライの 11 
のタイプを以下に示す。
...
\reqtypeqdic   & QueryDictionary   & C8 & C8 & C16& I32& I16& S8 & S8&& \\
\reqtypethrough& Through           & C8 & C8 & C16& I16& I32& I32& S8&& \\
...
※リクエストType16とType19、リプライType3とType7は、同じような気がしますが.....
...
クライアントがサーバに発行するリクエストは以下の通りである。バージョン
ごとに新規に追加したものに関しては、先頭に記号を付けて示している。
(記号とバージョンの対応は表の最後に記述)
...
\reqNoticeGroupName & \repNoticeGroupName \\
  ...
  {+++} & KillServer    	& サーバを終了させる       & 0x24 &
\reqKillServer & \repKillServer \\
\hline
\end{tabular}

※要求番号 0x23は、debug用関数 RkThrough()のために 追加された protocol
...
\\
\begin{itemize}
\item  プロトコルバージョン 2.0 で新規に追加 … 先頭に「*」マーク
\item  プロトコルバージョン 2.1 で新規に追加 … 先頭に「**」マーク
\item  プロトコルバージョン 3.0 で新規に追加 … 先頭に「+」マーク
\item  プロトコルバージョン 3.2 で新規に追加 … 先頭に「++」マーク
\item  プロトコルバージョン 3.3 で新規に追加 … 先頭に「+++」マーク
\end{itemize}
...
フィールド & フィールド長(octet) & タイプ & 備考 \\\hline\hline
    ...
    NONE       & 1 & CARD8 & 0x00 \\
    データ長 & 2 & CARD16 & 1 \\
    文節数\footnotemark & 1 & INT8 & エラー時: $-$1 \\
\hline
\end{tabular}

\footnotetext{他のプロトコルでは文節数にはINT16を用いているが、ここではINT8を用いる。}
\end{itemize}
\item 説明
...
リクエスト名リストには、1つの拡張をなす一連のリクエストの名前を指定する。
サーバから返される値は、その拡張の最初のリクエストの番号から1を減じた
ものである。拡張プロトコルが見つからない場合、$-$1が返される。

なお、Canna 3.6p3までのサーバでは、リクエスト名リストにはGetServerInfo
からCopyDictionaryまでを順に指定する必要があった。
%
\end{enumerate}
\newpage
%
...
\newpage
%
\subsection{サーバを終了させる: KillServer}
サーバに終了要求を通知し、サーバを終了させる。
\begin{enumerate}
\item パケット
\begin{itemize}
\item 要求パケット(\reqKillServer)

\begin{tabular}{|l|c|c|l|}\hline
フィールド & フィールド長(octet) & タイプ & 備考 \\\hline\hline
    KillServer        & 1 & CARD8  & 0x24 \\
    NONE	      & 1 & CARD8  & 0x00 \\
    データ長 	      & 2 & CARD16 & 0 \\
\hline
\end{tabular}

\item 応答パケット(\repKillServer)
...
本プロトコルは、サーバのプロトコルバージョンが 3.3 以降の場合、
用いることができる。
...
\begin{tabular}{l|l} 
値       & エラー内容 \\ \hline
OLDSRV   & 『かんな』Version 3.2以前のサーバにプロトコルを送った場合 \\
...
以上述べたプロトコルとRKライブラリとの対応は以下の通りである。
バージョンごとに新規に追加したものに関しては、先頭に記号を付けて示している。
(記号とバージョンの対応は表の最後に記述)
...
{+++}& Rk(w)KillServer	&+++& Killserver	 & サーバの終了 \\
\hline\hline
