...
%
% $Id: custom.tex,v 1.1.1.1.2.2 2003/09/17 01:47:03 aida_s Exp $
%
\chapter{カスタマイズ}
\label{3.3カスタマイズ}

かな漢字変換に関して変換に利用する辞書を始めとしてキー操作やローマ字か
な変換テーブルにいたるまでユーザの好みに合うように \HIDX{カスタマイズ}
{かすたまいす}{H0.4.0.0.0} することができます。 
...
ローマ字かな変換の規則を設定する&\hpref{count=11,label=H0.4.4.2.0}ローマ字かな変換ソース \\
                                     &  　　　　　　　　　　(×××.ctd) \\
...
\section{\HIDX{カスタマイズファイル}{かすたまいすふあいる}{H0.4.1.0.0}}
\label{3.3.1カスタマイズファイル}

カスタマイズはカスタマイズファイルを用いて行います。カ
スタマイズファイルは特別の指定がないかぎりホームディレクトリに
ある \HIDX{.canna}{．ＣＡＮＮＡ}{H0.4.1.0.0} というファイルが用いられます。

\subsection{カスタマイズのための初期設定}

カスタマイズを行うにはまず \refCANNALIBDIR /sample/default.canna
を自分のホームディレクトリに持ってきて \verb+.canna+ という名前でセー
ブします。
...
\subsection{カスタマイズファイルの指定}

環境変数の設定により参照するカスタマイズファイルを変更することが
できます。
...
\item 環境変数 \HIDX{CANNAFILE}{ＣＡＮＮＡＦＩＬＥ}{H0.4.1.2.0} が
設定されているとき
...
\refCANNALIBDIR /\HIDX{default.canna}{ＤＥＦＡＵＬＴ．ＣＡＮＮＡ}
{H0.4.1.2.0.a1} \hptar{label=H0.4.1.2.0.a1} がカスタマイズファイルとして用いられます。
...
\begin{nquote}{1em}
\begin{namelist}{　　　}
\item[{\dm \rm (2.1)}]環境変数 \HIDX{DISPLAY}{ＤＩＳＰＬＡＹ}{H0.4.1.2.0.a2} \hptar{label=H0.4.1.2.0.a2}に
値が設定されており、その値が ×××:0 であるときに、ホームディレクトリに 
~.canna-$\times\times\times$ という名前のファイルが存在する場合、さらに
このファイルも読み込まれます。

\item[{\dm \rm (2.2)}]環境変数 \HIDX{TERM}{ＴＥＲＭ}{H0.4.1.2.0.a3} \hptar{label=H0.4.1.2.0.a3}に
...
以上をまとめるとカスタマイズファイルのサーチ順は 表\ref{カスタマイズファ
イルのサーチ順} のとおりになります。 
...
%{\tt
\begin{table}[hbtp]
\caption{カスタマイズファイルのサーチ順}
\label{カスタマイズファイルのサーチ順}
\begin{center}
\begin{tabular}{|l|l|}
\hline
...
% ------------------------------ newnew
\subsection{サンプルとして提供するカスタマイズファイル}
\label{サンプルとして提供するカスタマイズファイル}

『かんな』以外のフロントエンドプロセッサになれていらっしゃる方もいるで
しょう。 そのような方のために、システムでは、他のフロントエンドプロセッ
サの操作性を有したカスタマイズファイル\raisebox{0.6ex}{\footnotesize
\dg 注 1} を提供しています。  

サンプルとして提供されているカスタマイズファイルとしては表\ref{提供するカ
スタマイズファイル} があります。
これらのサンプルファイルは \refCANNALIBDIR /sample の下にファイルの形
で存在します。%\raisebox{0.6ex}{\footnotesize \dg 注 2} 

default.canna がデフォルトの設定をカスタマイズファイルで表したものになっ
ています。
...
\begin{table}[hbtp]
\begin{center}
\caption{サンプルとして提供するカスタマイズファイル}\label{提供するカ
スタマイズファイル}
\begin{tabular}{|l|p{10cm}|}
\hline
\multicolumn{1}{|c|}{ファイル名} &  \multicolumn{1}{|c|}{備　　　　　　　考} \\
\hline
default.canna  & デフォルトの設定をカスタマイズファイルで表したもの \\
\hline
sample.canna & カスタマイズの方法の例を記述したカスタマイズファイル\\
\hline
just.canna    & 一太郎 \footnotemark とほぼ同じキー操作を提供するカス
タマイズファイル \\ 
\hline 
matsu.canna     & 松茸\footnotemark とほぼ同じキー操作を提供するカスタ
マイズファイル \\ 
\hline
vje.canna     & vje$-\beta$\footnotemark とほぼ同じキー操作を提供するカス
タマイズファイル \\ 
\hline 
wx2+.canna     & WXII+\footnotemark とほぼ同じキー操作を提供するカスタマ
イズファイル \\ 
\hline 
verbose.canna  & カスタマイズ可能な全キーワードを列挙したカスタマイズ
ファイル \\ 
\hline 
\end{tabular}
\end{center}
...
\addtocounter{footnote}{-3}\footnotetext{一太郎はジャストシステム(株)
の商標です} 
\addtocounter{footnote}{1}\footnotetext{松茸は(株)管理工学研究所の商標です} 
\addtocounter{footnote}{1}\footnotetext{VJE$-\beta$は(株)バックスの商標です}
\addtocounter{footnote}{1}\footnotetext{WXII+はエー・アイ・ソフト(株)
の商標です} 

サンプルのカスタマイズファイルを使用したい場合は、環境変数 CANNAFILE 
を設定する$^{(1)}$か、自分のホームディレクトリ にサンプルファイルを 
.canna としてコピーして使用する$^{(2)}$とよいでしょう。
just.canna を使用したい場合は次のいずれかを実行します。

\vspace{3mm}

\begin{nquote}{3em}
\begin{enumerate}
\item \texttt{\% setenv  CANNAFILE  \CANNALIBDIR/sample/just.canna}
\item \texttt{\% cp  \CANNALIBDIR/sample/just.canna  \textasciitilde/.canna}
\end{enumerate}
\end{nquote}
...
\begin{nquote}{1em}
\begin{namelist}{　}
\item [注 1] カスタマイズファイルは、できるかぎり各々のフロントエン
ドプロセッサのキー操作と同じになるように設定してありますが、X ウィンドウ 
においてのみ機能するキー (表 \ref{Xウィンドウのみのキー一覧} 参照) なども
存在するため、必ずしも、全く同じ操作方法が実現できるわけではありません。 

%% \item [注 2] サンプルファイルは、cannasmp パッケージに含まれています。
%% \refCANNALIBDIR /sample が存在しない場合は、 cannasmp パッケージをイン
%% ストールしてください。
\end{namelist}
\end{nquote}

\subsection{カスタマイズファイルの例}

次頁に\HIDX{カスタマイズファイルの例}{かすたまいすふあいるのれい}
{H0.4.1.4.0} を示します。

カスタマイズファイルは \HIDX{Lisp}{ＬＩＳＰ}{H0.4.1.4.0} 言語と同様の
シンタックスにより記述します。
...
:katakana                & 自動登録用辞書 & カタカナ語を自動登録する辞書を指定します。 \\ \hline
\end{tabular}
\end{center}
\end{table}
%}
...
指定できるシステム辞書としては表\ref{指定できる辞書}に示すものがあります。
...
\hline
 %%%%% %%%%%% %%%%% %%%%%% %%%%% %%%%%%
 %iroha     & 基幹辞書(8万語) \\
iroha     & \HIDX{基幹辞書}{きかんししよ}{H0.4.2.0.0.a} \hptar{label=H0.4.2.0.0.a} \\
\hline
fuzokugo  & \HIDX{付属語辞書}{ふそくこししよ}{H0.4.2.0.0.a} \\
\hline
bushu     & \HIDX{部首辞書}{ふしゆししよ}{H0.4.2.0.0.a} \\
\hline
yuubin    & \HIDX{郵便番号辞書}{ゆうひんはんこうししよ}{H0.4.2.0.0.a} \\
\hline
...
fuzokugo.cbd に記述されている文法情報を使用します。
...
参考用としてシステムファイル(\refCANNALIBDIR /dic/canna)に
含まれている辞書を表\ref{テキスト形式システム辞書}に示します。
...
\begin{table}[hbtp]
\begin{center}
\caption{\refCANNALIBDIR /dic/canna ディレクトリにあるテキスト形式システム辞書}\label{テキスト形式システム辞書}
\begin{tabular}{|l|l|}
\hline
\multicolumn{1}{|c|}{辞 書 名} & \multicolumn{1}{|c|}{説　　　明} \\
\hline
...
\begin{nquote}{1em}
\begin{namelist}{　}
\item[注意]デフォルトカスタマイズファイル(\refCANNALIBDIR/default.canna)では、単語登録用辞書としてあらかじめ "user" が指定されています。
\end{namelist}
\end{nquote}

キーワード \HIDX{:katakana}{：ＫＡＴＡＫＡＮＡ}{H0.4.2.0.0} で指定されるカタカ
ナ語自動登録辞書は特殊な辞書です。字種変換（\textbf{\ref{字種変換による入力}
字種変換による入力}参照）で全角カタカナに確定したときに、そのカタカナ
を自動登録することができます。自動登録の指定は、カスタマイズファイル中
で以下のように記述します。
...
自動登録用の辞書はテキスト形式で、書き込み可能になっていなければなりま
せん（\textbf{\ref{辞書のREAD・WRITE権}辞書のREAD・WRITE権}参照）。自
動登録機能を使うときは、あらかじめ単語登録用辞書と同様に
\texttt{mkdic} コマンドで辞書を作っておくことをおすすめします。
...
表 \ref{提供するローマ字辞書テーブル} のものがあります。 これらのテー
ブルは \refCANNALIBDIR /dic の下にファイルの形で存在します。 
...
\hline
default.cbp  & デフォルトのローマ字かな変換テーブル \\
\hline
 %%%%% %%%%% %%%%% %%%%% %%%%% %%%%%
 %jdaemon.cbp  & EWS-UX/V R6.×以前の日本語入力システムと互換のローマ字
 %かな変換テーブル \\ 
 %\hline
 %%%%% %%%%% %%%%% %%%%% %%%%% %%%%%
just.cbp     & 一太郎\footnotemark と同一のローマ字かな変換規則を持つロー
マ字かな変換テー ブル \\
\hline
matsu.cbp     & 松茸\footnotemark と同一のローマ字かな変換規則を持つロー
マ字かな変換テーブル \\ 
\hline
vje.cbp     & vje$-\beta$\footnotemark と同一のローマ字かな変換規則を持
つローマ字かな変換 テーブル \\ 
\hline
wx2+.cbp     & WXII+\footnotemark と同一のローマ字かな変換規則を持つロー
マ字かな変換テーブル \\
\hline
kana.cbp     & アルファベットキーボードで疑似的に
かな入力を行うためのテーブル。このテーブルを用いることにより、
日本語モードとアルファベットモードの切り替えだけで、
かな入力とアルファベット入力の切り替えも行えます。 \\
\hline
newjis.cbp   & アルファベットキーボードで疑似的に新JISキーボードのカナ配列をシミュレートするためのテーブル \\
\hline
kaisoku.cbp  & 快速ローマ字配列をシミュレートするためのテーブル \\
\hline
\end{tabular}
\end{center}
\end{table}
...
の商標です}
\addtocounter{footnote}{1}\footnotetext{松茸は(株)管理工学研究所の商標です}  
\addtocounter{footnote}{1}\footnotetext{VJE$-\beta$は(株)バックスの商
標です}
\addtocounter{footnote}{1}\footnotetext{WXII+はエー・アイ・ソフト(株) 
の商標です}  
...
デフォルトではdefault.cbpが選ばれますが、そのほかのローマ字かな変換テーブルを
利用する場合はカスタマイズファイルに次のような行を加えます。
...
カスタマイズする場合は、デフォルトのローマ字かな変換テーブルのソースファ
イルである \\ 
\refCANNALIBDIR /sample/src/default.ctd をコピーして書き換えるのが簡
単です。 
ローマ字かな変換テーブルは1行にローマ字とそれに対応する『かな』とが記
述されているテーブルです。表 \ref{ローマ字かな変換テーブルの内容} のよ
うな内容になっています。

\begin{table}[htbp]
\centering
\caption{ローマ字かな変換テーブルの内容}
\label{ローマ字かな変換テーブルの内容}
\begin{tabular}{|lll|}
\hline
a   & あ       &     \\
i   & い       &     \\
...
たとえば、「べんりな」と入力するときは 図 \ref{最長一致法の原則} のようにテーブルが参照されます。
...
カスタマイズファイルを用いて、かな漢字変換に用いられるキーの割り当てを
...
記述することにより行います。
主な機能名としては 表\ref{主な機能名} のものがあります。\\
そのほかの機能名に関しては
{\dg 付録 \ref{Cカスタマイズに用いる機能名一覧表} 
\hpref{count=16,label=H0.C.0.0.0}カスタマイズに用いる機能名一覧表} を
参照してください。
...
\hline
\HIDX{alpha-mode}{ＡＬＰＨＡ−ＭＯＤＥ}{H0.4.5.2.0.a} \hptar{label=H0.4.5.2.0.a}  
		& アルファベットモードに移行する& Xfer,C-o & \\
\hline
\HIDX{quoted-insert}{ＱＵＯＴＥＤ−ＩＮＳＥＲＴ}{H0.4.5.2.0.a} 
		& 次の一文字を無条件に入力する  & C-q & 　\\
\hline
\HIDX{kakutei}{ＫＡＫＵＴＥＩ}{H0.4.5.2.0.a} 
		& 確定する                      & Return, Nfer & 　\\
\hline
\HIDX{extend}{ＥＸＴＥＮＤ}{H0.4.5.2.0.a} 
		& 領域伸ばし                    & S-→, C-o & 　\\
\hline
\HIDX{shrink}{ＳＨＲＩＮＫ}{H0.4.5.2.0.a} 
		& 領域縮め                      & S-←, C-i & 　\\
\hline
\HIDX{touroku}{ＴＯＵＲＯＫＵ}{H0.4.5.2.0.a} 
		& 単語登録                      & Help & 　\\
\hline
\HIDX{forward}{ＦＯＲＷＡＲＤ}{H0.4.5.2.0.a} 
		& 右選択                        & →, C-f & 　\\
\hline
\HIDX{backward}{ＢＡＣＫＷＡＲＤ}{H0.4.5.2.0.a} 
		& 左選択                        & ←, C-b & 　\\
\hline
\HIDX{previous}{ＰＲＥＶＩＯＵＳ}{H0.4.5.2.0.a} 
		& 前選択                        & ↑, C-p & 　\\
\hline
\HIDX{next}{ＮＥＸＴ}{H0.4.5.2.0.a} 
		& 次選択                        & ↓, C-n & 　\\
\hline
\HIDX{beginning-of-line}{ＢＥＧＩＮＮＩＮＧ−ＯＦ−ＬＩＮＥ}{H0.4.5.2.0.a} 
		& 先頭選択                      & C-a & 　\\
\hline
\HIDX{end-of-line}{ＥＮＤ−ＯＦ−ＬＩＮＥ}{H0.4.5.2.0.a} 
		& 末尾選択                      & C-e & 　\\
\hline
\HIDX{delete-next}{ＤＥＬＥＴＥ−ＮＥＸＴ}{H0.4.5.2.0.a} 
		& 次一文字消去                  & C-d & 　\\
\hline
\HIDX{delete-previous}{ＤＥＬＥＴＥ−ＰＲＥＶＩＯＵＳ}{H0.4.5.2.0.a} 
		& 前一文字消去                  & Backspace & 　\\
\hline
\HIDX{kill-to-end-of-line}{ＫＩＬＬ−ＴＯ−ＥＮＤ−ＯＦ−ＬＩＮＥ}{H0.4.5.2.0.a} 
		& 行末まで消去                  & C-k & 　\\
\hline
\HIDX{bubun-kakutei}{ＢＵＢＵＮ−ＫＡＫＵＴＥＩ}{H0.4.5.2.0.a} 
		& カーソル前まで確定            & C-j & 　\\
\hline
\HIDX{henkan}{ＨＥＮＫＡＮ}{H0.4.5.2.0.a} 
		& 変換                          & Space, Xfer & 　\\
\hline
\HIDX{quit}{ＱＵＩＴ}{H0.4.5.2.0.a}
		& 取りやめ                      & C-g & 　\\
\hline
\HIDX{self-insert}{ＳＥＬＦ−ＩＮＳＥＲＴ}{H0.4.5.2.0.a} 
		& 文字挿入                      & a, b, c … & 　\\
\hline
\end{tabular}
\end{center}
...
\HIDX{キー}{きー}{H0.4.5.3.0.a1} \hptar{label=H0.4.5.3.0.a1}は \verb!"a"! や \verb!"b"! のように指定します。
コントロールキーを押しながら何らかのキーを押すというのは
キーの指定の前に \verb!"\C-"! を付けることで表現します。
ファンクションキーなどの\HIDX{特殊キー}{とくしゆきー}{H0.4.5.3.0.a2} \hptar{label=H0.4.5.3.0.a2}については表
...
ROLLUP(PAGEDOWN)  & \tt "$\backslash$Rollup" \\
ROLLDOWN(PAGEUP)  & \tt "$\backslash$Rolldown" \\
↑                & \tt "$\backslash$Up" \\
←                & \tt "$\backslash$Left" \\
→                & \tt "$\backslash$Right" \\
↓                & \tt "$\backslash$Down" \\
...
\HIDXAS{X ウィンドウのキー}{Ｘういんとうのきー}{H0.4.5.3.0.a3}
\hptar{label=H0.4.5.3.0.a3}
%{\tt
{\small
\begin{table}[hbtp]
\begin{center}
...
ひらがな  & \tt "$\backslash$Hiragana" \\
カタカナ  & \tt "$\backslash$Katakana" \\
半角/全角 & \tt "$\backslash$Hankakuzenkaku" \\
英数      & \tt "$\backslash$Eisu" \\
\hline
\end{tabular}
\end{center}
...
そのキーと\HIDX{等価なキー}{とうかなきー}{H0.4.5.3.0.a4} \hptar{label=H0.4.5.3.0.a4}にも
機能を割り当てたことになります。
...
\HIDX{モード名}{もーとめい}{H0.4.5.3.0.a5} \hptar{label=H0.4.5.3.0.a5}は
...
\hline
\HIDX{alpha-mode}{ＡＬＰＨＡ−ＭＯＤＥ}{H0.4.5.3.0.b} \hptar{label=H0.4.5.3.0.b} 
		& アルファベットを入力している状態  \\ \hline
\HIDX{empty-mode}{ＥＭＰＴＹ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 日本語入力モードでまだ文字を入力していない状態\\ \hline
\HIDX{yomi-mode}{ＹＯＭＩ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 読みを入力している状態\\ \hline
\HIDX{mojishu-mode}{ＭＯＪＩＳＨＵ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 文字種変換を行っている状態\\ \hline
\HIDX{tankouho-mode}{ＴＡＮＫＯＵＨＯ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 変換キーを押して候補を表示している状態\\ \hline
\HIDX{ichiran-mode}{ＩＣＨＩＲＡＮ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 候補一覧表示をしている状態\\ \hline
\HIDX{henkan-nyuuryoku-mode}{ＨＥＮＫＡＮ−ＮＹＵＵＲＹＯＫＵ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& empty-modeとyomi-modeの総称\\ \hline 
\HIDX{kigou-mode}{ＫＩＧＯＵ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 記号一覧を入力している状態\\ \hline
\HIDX{yes-no-mode}{ＹＥＳ−ＮＯ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& YES/NOを問い合わせる質問をしている状態\\ \hline
\HIDX{on-off-mode}{ＯＮ−ＯＦＦ−ＭＯＤＥ}{H0.4.5.3.0.b}
		& 辞書マウントアンマウントを指定している状態\\ \hline
\HIDX{shinshuku-mode}{ＳＨＩＮＳＨＵＫＵ−ＭＯＤＥ}{H0.4.5.3.0.b}
...
\HIDXAS{global-unbind-key-function}{ＧＬＯＢＡＬ−ＵＮＢＩＮＤ−ＫＥＹ
−ＦＵＮＣＴＩＯＮ}{H0.4.5.5.0.a1}
\hptar{label=H0.4.5.5.0.a1}
\begin{nquote}{3em}
\begin{verbatim}
...
\HIDXAS{unbind-key-function}{ＵＮＢＩＮＤ−ＫＥＹ−ＦＵＮＣＴＩＯＮ}
{H0.4.5.5.0.a2}
\hptar{label=H0.4.5.5.0.a2}
\begin{nquote}{3em}
\begin{verbatim}
...
引数は一度「\HIDX{評価}{ひようか}{H0.4.6.1.0.a1} \hptar{label=H0.4.6.1.0.a1}」
(\HIDX{evaluate}{ＥＶＡＬＵＡＴＥ}{H0.4.6.1.0.a2} \hptar{label=H0.4.6.1.0.a2})されてから
...
{H0.4.6.4.0.1} $-$ \HIDX{use-dictionary}{ＵＳＥ−ＤＩＣＴＩＯＮＡＲＹ}
{H0.4.6.4.0.1}}
...
% misao … 文法辞書追加 %
部首辞書、文法辞書、単語登録用辞書、カタカナ語自動登録用辞書を指定する
には辞書の 
名前の直前に \HIDX{:bushu}{：ＢＵＳＨＵ}{H0.4.6.4.0.1} や 
\HIDX{:grammar}{：ＧＲＡＭＭＡＲ}{H0.4.6.4.0.1}, 
\HIDX{:user}{：ＵＳＥＲ}{H0.4.6.4.0.1}, \HIDX{:katakana}{：ＫＡＴＡＫＡＮＡ}{H0.4.6.4.0.1} を
先行させます。
...
　　　　　　　　:katakana "mykatakana"　:grammar "mygram" ) \\
　　　　　　　　　　　　　　　　　　　　:user "mydic" ) \\
\end{CODEBOX}

\begin{nquote}{1em}
\begin{namelist}{　}
\item[注意] :bushu, :grammar, :user, :katakana はシンボルなので
...
(use-dictionary \\
"iroha" "fuzokugo" :bushu "bushu" :katakana "mykatakana" :grammar "mygram" :user "mydic") \\ \hline
\end{tabular}
\end{center}
\vspace{5mm}
...
\HIDXAS{モード名}{もーとめい}{H0.4.6.4.0.2.a}
\hptar{label=H0.4.6.4.0.2.a} 
...
\newpage
%%%%%
\HIDXAS{モード一覧}{もーといちらん}{H0.4.6.4.0.2.b}
...
\hline
\HIDX{alpha-mode}{ＡＬＰＨＡ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 &          &  アルファベット入力モード \\
\hline
\HIDX{henkan-nyuuryoku-mode}{ＨＥＮＫＡＮ−ＮＹＵＵＲＹＯＫＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b} 
		 & [ あ ]   &  変換入力モード \\
\hline
\HIDX{kigou-mode}{ＫＩＧＯＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b} \hptar{label=H0.4.6.4.0.2.b} 
		 & [記号]   &  記号一覧表示状態 \\
\hline
\HIDX{yomi-mode}{ＹＵＭＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & nil      &  読みを入力している状態 \\
\hline
\HIDX{mojishu-mode}{ＭＯＪＩＳＨＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [字種]   &  文字種変換状態 \\
\hline
\HIDX{tankouho-mode}{ＴＡＮＫＯＵＨＯ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [漢字]   &  単候補表示状態 \\
\hline
\HIDX{ichiran-mode}{ＩＣＨＩＲＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [一覧]   &  候補一覧表示状態 \\
\hline
\HIDX{yes-no-mode}{ＹＥＳ−ＮＯ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [質問]   &  利用者に質問をしている状態 \\
\hline
\HIDX{on-off-mode}{ＯＮ−ＯＦＦ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & nil      &  辞書マウントアンマウント状態など \\
\hline
\HIDX{shinshuku-mode}{ＳＨＩＮＳＨＵＫＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [文節]   &  文節伸縮モード \\
\hline
\HIDX{chikuji-yomi-mode}{ＣＨＩＫＵＪＩ−ＹＯＭＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [逐次]   &  逐次自動入力時の読み部分 \\
\hline
\HIDX{chikuji-bunsetsu-mode}{ＣＨＩＫＵＪＩ−ＢＵＮＳＥＴＳＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [逐次]   &  逐次自動入力時の文節部分 \\
\hline
\HIDX{zen-hira-henkan-mode}{ＺＥＮ−ＨＩＲＡ−ＨＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [全あ]   &  全角ひらがなモード \\
\hline
\HIDX{han-hira-henkan-mode}{ＨＡＮ−ＨＩＲＡ−ＨＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [半あ]   &  半角ひらがなモード \\
\hline
\HIDX{zen-kata-henkan-mode}{ＺＥＮ−ＫＡＴＡ−ＨＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [全ア]   &  全角カタカナモード \\
\hline
\HIDX{han-kata-henkan-mode}{ＨＡＮ−ＫＡＴＡ−ＨＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [半ア]   &  半角カタカナモード \\
\hline
\HIDX{zen-alpha-henkan-mode}{ＺＥＮ−ＡＬＰＨＡ−ＨＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [全英]   &  全角英数モード \\
\hline
\HIDX{han-alpha-henkan-mode}{ＨＡＮ−ＡＬＰＨＡ−ＫＥＮＫＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [半英]   &  半角英数モード \\
\hline
\HIDX{zen-hira-kakutei-mode}{ＺＥＮ−ＨＩＲＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$全あ$>$   &  全角ひらがな確定モード \\
\hline
\HIDX{han-hira-kakutei-mode}{ＨＡＮ−ＨＩＲＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$半あ$>$   &  半角ひらがな確定モード \\
\hline
\HIDX{zen-kata-kakutei-mode}{ＺＥＮ−ＫＡＴＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$全ア$>$   &  全角カタカナ確定モード \\
\hline
\HIDX{han-kata-kakutei-mode}{ＨＡＮ−ＫＡＴＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$半ア$>$   &  半角カタカナ確定モード \\
\hline
\HIDX{zen-alpha-kakutei-mode}{ＺＥＮ−ＡＬＰＨＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$全英$>$   &  全角英数確定モード \\
\hline
\HIDX{han-alpha-kakutei-mode}{ＨＡＮ−ＡＬＰＨＡ−ＫＡＫＵＴＥＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & $<$半英$>$   &  半角英数確定モード \\
\hline
\HIDX{hex-mode}{ＨＥＸ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [16進]   &  コード入力状態 \\
\hline
\HIDX{bushu-mode}{ＢＵＳＨＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [部首]   &  部首入力モード \\
\hline
\HIDX{extend-mode}{ＥＸＴＥＮＤ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [拡張]   &  ユーティリティモード \\
\hline
\HIDX{russian-mode}{ＲＵＳＳＩＡＮ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [~ロ~]   &  ロシア文字一覧状態 \\
\hline
\HIDX{greek-mode}{ＧＲＥＥＫ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [~ギ~]   &  ギリシャ文字一覧状態 \\
\hline
\HIDX{line-mode}{ＬＩＮＥ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [罫線]   &  罫線一覧状態 \\
\hline
\HIDX{changing-server-mode}{ＣＨＡＮＧＩＮＧ−ＳＥＲＶＥＲ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [変更]   &  サーバ変更状態 \\
\hline
\HIDX{henkan-method-mode}{ＨＥＮＫＡＮ−ＭＥＴＨＯＤ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [変換]   &  変換方法変更状態 \\
\hline
\HIDX{delete-dic-mode}{ＤＥＬＥＴＥ−ＤＩＣ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [削除]   &  単語削除状態 \\
\hline
\HIDX{touroku-mode}{ＴＯＵＲＯＫＵ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [登録]   &  単語登録状態 \\
\hline
\HIDX{touroku-hinshi-mode}{ＴＯＵＲＯＫＵ−ＨＩＮＳＨＩ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [品詞]   &  品詞選択状態 \\
\hline
\HIDX{touroku-dic-mode}{ＴＯＵＲＯＫＵ−ＤＩＣ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [辞書]   &  辞書選択状態 \\
\hline
\HIDX{quoted-insert-mode}{ＱＵＯＴＥＤ−ＩＮＳＥＲＴ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
		 & [~ｑ~]   &  引用入力状態 \\
\hline
\HIDX{mount-dic-mode}{ＭＯＵＮＴ−ＤＩＣ−ＭＯＤＥ}{H0.4.6.4.0.2.b}
...
{H0.4.6.4.0.3} $-$ \HIDX{set-key}{ＳＥＴ−ＫＥＹ}{H0.4.6.4.0.3},
\HIDX{global-set-key}{ＧＬＯＢＡＬ−ＳＥＴ−ＫＥＹ}{H0.4.6.4.0.3}}
...
C-\textbackslash を表現するときは \texttt{"\textbackslash
  C-\textbackslash"} と記述します。

Xfer など特殊なキーに関しては以下のように記述します。
\HIDXAS{特殊キー}{とくしゆきー}{H0.4.6.4.0.3}
...
\verb+ "\Return"   "\End"      "\F1"       "\F2"       "\F3"       "\F4" + \\
\verb+ "\F5"       "\F6"       "\F7"       "\F8"       "\F9"       "\F10" + \\
\verb+ "\Pf1"      "\Pf2"      "\Pf3"      "\Pf4"      "\Pf5"      "\Pf6" + \\
\verb+ "\Pf7"      "\Pf8"      "\Pf9"      "\Pf10"     "\S-Nfer"   "\S-Xfer" + \\
\verb+ "\S-Up"     "\S-Down"   "\S-Left"   "\S-Right"  "\C-Nfer"   "\C-Xfer" + \\
\verb+ "\C-Up"     "\C-Down"   "\C-Left"   "\C-Right"  "\Hiragana" "\Katakana" + \\
\verb+ "\Eisu"     "\Hankakuzenkaku" + \\
\hline
\end{tabular}
\end{center}
}
...
{H0.4.6.4.0.4} $-$ \HIDX{unbind-key-function}
{ＵＮＢＩＮＤ−ＫＥＹ−ＦＵＮＣＴＩＯＮ}{H0.4.6.4.0.4}, 
\HIDX{global-unbind-key-function}
{ＧＬＯＢＡＬ−ＵＮＢＩＮＤ−ＫＥＹ−ＦＵＮＣＴＩＯＮ}{H0.4.6.4.0.4}}
...
　　;カナ入力モード(疑似的な)を定義します。 \\
　　(defmode kana-input-mode "[カナ]" "kana.cbp") \\
　　;C-rでトグルするようにします。 \\
...
\item {\dg\bf \HIDX{記号定義}{きこうていき}{H0.4.6.4.0.6} $-$ 
\HIDX{defsymbol}{ＤＥＦＳＹＭＢＯＬ}{H0.4.6.4.0.6}}
\label{カスタマイズ・記号定義}
...
defsymbol では、入力キーに対して他のキーが入力されたように振舞わせるこ
ともできます。

\begin{CODEBOX}
(例) F1 を押したら全角スペースを入力する。その時入力キーはあたかも
         スペースキーであったかのようにする。\\
\verb!         (defsymbol ?\F1 ?\Space "　")! \\
\end{CODEBOX}

% ------------------------------ (7)
\hptar{label=H0.4.6.4.0.7}
\item {\dg\bf \HIDX{メニューの定義}{めにゆーのていき}{H0.4.6.4.0.7} $-$ 
\HIDX{defmenu}{ＤＥＦＭＥＮＵ}{H0.4.6.4.0.7}}
...
{\dg\bf (\ref{カスタマイズ・モード表示の変更}) 
\hpref{count=8,label=H0.4.6.4.0.2}モード表示の変更} を参照してください。

表示文字列には、そのメニューの一覧表示で表示される候補名を指定します。

機能シンボルには、表\ref{メニュー}に示す機能名、または defselection で定義した
機能シンボルを指定します。defselection については、
{\dg\bf (\ref{カスタマイズ・文字一覧の作成}) 
\hpref{count=7,label=H0.4.6.4.0.8}文字一覧の作成} を参照してください。
...
\hline
\HIDX{kigou-mode}{ＫＩＧＯＵ−ＭＯＤＥ}{H0.4.6.4.0.7.a} \hptar{label=H0.4.6.4.0.7.a} 
		 & 記号全般の一覧モードになる\\ 
\hline
\HIDX{russian-mode}{ＲＵＳＳＩＡＮ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & ロシア文字一覧モードになる\\
\hline
\HIDX{greek-mode}{ＧＲＥＥＫ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & ギリシャ文字一覧モードになる\\
\hline
\HIDX{line-mode}{ＬＩＮＥ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 罫線一覧モードになる\\
\hline
\HIDX{hex-mode}{ＨＥＸ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 16 進コード入力モードになる\\
\hline
\HIDX{bushu-mode}{ＢＵＳＨＵ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 部首入力モードになる\\
\hline
\HIDX{touroku-mode}{ＴＯＵＲＯＫＵ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 単語登録モードになる\\
\hline
\HIDX{delete-dic-mode}{ＤＥＬＥＴＥ−ＤＩＣ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 単語削除モードになる\\
\hline
\HIDX{jisho-ichiran}{ＪＩＳＨＯ−ＩＣＨＩＲＡＮ}{H0.4.6.4.0.7.a}
		 & 辞書マウント／アンマウントを行う\\
\hline
\HIDX{chikuji-mode}{ＣＨＩＫＵＪＩ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 逐次自動変換に切り替える\\
\hline
\HIDX{renbun-mode}{ＲＥＮＢＵＮ−ＭＯＤＥ}{H0.4.6.4.0.7.a}
		 & 連文節変換に切り替える\\
\hline
\HIDX{disconnect-server}{ＤＩＳＣＯＮＮＥＣＴ−ＳＥＲＶＥＲ}{H0.4.6.4.0.7.a}
		 & サーバとの接続を切る\\
\hline
\HIDX{switch-server}{ＳＷＩＴＣＨ−ＳＥＲＶＥＲ}{H0.4.6.4.0.7.a}
		 & サーバの切り替えを行う\\
\hline
\HIDX{show-server-name}{ＳＨＯＷ−ＳＥＲＶＥＲ−ＮＡＭＥ}{H0.4.6.4.0.7.a}
		 & サーバ名を表示する\\
\hline
\HIDX{show-gakushu}{ＳＨＯＷ−ＧＡＫＵＳＨＵ}{H0.4.6.4.0.7.a}
		 & 学習状態を表示する\\
\hline
\HIDX{show-canna-version}{ＳＨＯＷ−ＣＡＮＮＡ−ＶＥＲＳＩＯＮ}{H0.4.6.4.0.7.a}
		 & バージョンを表示する\\
\hline
\HIDX{show-romkana-table}{ＳＨＯＷ−ＲＯＭＫＡＮＡ−ＴＡＢＬＥ}{H0.4.6.4.0.7.a}
		 & ローマ字かな変換テーブル名を表示する\\
\hline
\HIDX{show-canna-file}{ＳＨＯＷ−ＣＡＮＮＡ−ＦＩＬＥ}{H0.4.6.4.0.7.a}
		 & カスタマイズファイル名を表示する\\
\hline
\HIDX{sync-dictionary}{ＳＹＮＣ−ＤＩＣＴＩＯＮＡＲＹ}{H0.4.6.4.0.7.a}
		 & 辞書に書き込む\\
\hline
\end{tabular}
\end{center}
...
{\dg\bf (\ref{カスタマイズ・キーのバインド}) 
\hpref{count=7,label=H0.4.6.4.0.3}キーのバインド} を参照してください。
...
カスタマイズファイルのメニューの設定にエラーが存在した場合、
そのメニューを選択したとき、
ガイドラインに "この項目は正しく定義されていません" と表示して、
empty モードに戻ります。
...
\item {\dg\bf \HIDX{文字一覧の作成}{もしいちらんのさくせい}{H0.4.6.4.0.8} $-$ 
\HIDX{defselection}{ＤＥＦＳＥＬＥＣＴＩＯＮ}{H0.4.6.4.0.8}}
...
\begin{nquote}{3em}
\begin{enumerate}
\item \label{string} 各要素を文字列として指定する
(ダブルクォーテーションで囲む)。

\item \label{character} 各要素を文字として指定する
(要素の前にクエスチョンマークをつける)。
\end{enumerate}
\end{nquote}

(\ref{string}) の場合は、各要素をすべて指定します。
...
{H0.4.6.4.0.9} $-$ \HIDX{initialize-function}{ＩＮＩＴＩＡＬＩＺＥ−Ｆ
ＵＮＣＴＩＯＮ}{H0.4.6.4.0.9}}
...
いるのよみこみ}{H0.4.6.4.0.10} $-$ \HIDX{load}{ＬＯＡＤ}{H0.4.6.4.0.10}}
...
たとえば『かんな』Version3.3 では canna-version には 3003 が入っています。
次のように利用することができます。
...
−ＰＨＯＮＯＧＲＡＭ}{H0.4.6.4.0.101}

ローマ字かな変換で不正なローマ字が入力として残るかどうかを指定します。
デフォルトでは不正なローマ字は残りますが、abandon-illegal-phonogram に 
t を 指定すると不正なローマ字は入力から捨てられます。
デフォルトは nil です。
...
\item \HIDX{allow-next-input}{ＡＬＬＯＷ−ＮＥＸＴ−ＩＮＰＵＴ}{H0.4.6.4.0.102}

候補一覧表示状態で数字以外のキーを押したときに、現在カーソルが位置して
い る候補が選択され次の入力となるかどうかを指定します。
allow-next-input が t の場合、候補一覧表示状態で数字以外のキーを打つと、
次の 入力となります。
allow-next-input が nil の場合、「ピッ」という音が鳴り、次の入力には用
いられません。
デフォルトは t です。
...
\item \HIDX{auto}{ＡＵＴＯ}{H0.4.6.4.0.103}
...
\item \HIDX{auto-sync}{ＡＵＴＯ−ＳＹＮＣ}{H0.4.6.4.0.104}

単語登録削除した直後に自動的に辞書の書き出し処理を行うかどうかを指定し
ます。 auto-sync が t の場合は、自動的に辞書の書き出し処理を行います。
auto-sync が nil の場合は、辞書に対するアクセスがなくなった時点で辞書
の書き出し処理が行われます。 デフォルトは t です。
...
\item \HIDX{break-into-roman}{ＢＲＥＡＫ−ＩＮＴＯ−ＲＯＭＡＮ}
{H0.4.6.4.0.105} 
...
読みを入力しているときにカーソル移動を行う場合に、文字単位で移動を行う
かどうかを指定します。デフォルトは t です。character-based-move を nil
にする と、ローマ字かな変換の確定の単位をもとにしてカーソルを移動しま
す。すなわち、"ju" と入力した場合は、『じゅ』は一文字とみなされてカー
ソルが移動します。文字削除のときも同様に取り扱われます。
...
% ------- ・
\hptar{label=H0.4.6.4.0.107.5}
\item \HIDX{code-input}{ＣＯＤＥ−ＩＮＰＵＴ}{H0.4.6.4.0.107.5}

コード変換の機能を利用してコード入力を行う場合、JIS コード(``jis'')、シ
フト JIS コード(``sjis'')、区点コード(``kuten'')の 3 つのコード種別のいずれ
かを指定することができます。デフォルトは ``jis'' です。
...
\item \HIDX{cursor-wrap}{ＣＵＲＳＯＲ−ＷＲＡＰ}{H0.4.6.4.0.108}

読みを入力している状態や候補を表示している状態でカーソルを移動するとき
に右端から右へ移動する操作をしたときや左端から左や移動する操作をしたと
きに反対側の端にカーソルが移動することを指定します。t で移動し、nil で
移動しません。デフォルトは t です。
...
\begin{verbatim}
(例)
    (define-esc-sequence "kterm" "\Escape[28~" ?\Help)
    (define-esc-sequence "kterm" "\Escape[210z" ?\Xfer)
\end{verbatim}
\end{nquote}

ターミナル名とエスケープシーケンスは文字列を許しますが、最後のキーは単
キーとするため、文字を指定するように構文上の制約を加えています。 

% ------- ・
\hptar{label=H0.4.6.4.0.109.5}
\item \HIDX{delay-connect}{ＤＥＬＡＹ−ＣＯＮＮＥＣＴ}{H0.4.6.4.0.109.5}

最初の変換の時までサーバへの接続を遅延するかどうかを指定します。
 nil のときは初期化時にすぐに接続します。デフォルトは nil です。

\begin{nquote}{2em}
\begin{verbatim}
(例)
    (setq delay-connect nil)
\end{verbatim}
\end{nquote}

% ------- ・
\hptar{label=H0.4.6.4.0.110}
\item \HIDX{gakushu}{ＧＡＫＵＳＨＵ}{H0.4.6.4.0.110}
...
逐次自動変換を用いている場合、ベース切替え機能を利用してベースを切替え
た際のキーの動作を指定します。

たとえば、句点の入力時に変換を行うように指定してあるが、ベースがひらが
な以外の場合は変換を行わないようにするためには以下のように記述します。
...
\item \HIDX{henkan-or-self-insert}{ＨＥＮＫＡＮ−ＯＲ−ＳＥＬＦ−ＩＮ
ＳＥＲＴ} {H0.4.6.4.0.113}

連文節変換を用いている場合、ベース切替え機能を利用してベースを切替えた
際のキーの動作を指定します。

たとえば、ベース切替え機能を利用して、bese-eisu 機能により英数入力モー
ドに移行する際、入力ベースがひらがなの場合は変換を行い、ひらがな以外の
場合は self-insert を行うためには以下のように記述します。 
...
\item \HIDX{hex-direct}{ＨＥＸ−ＤＩＲＥＣＴ}{H0.4.6.4.0.114}

16 進コード入力時に 4 ケタ目を入れた時点で 16 進コード入力モードが終了
するかどうかを指定します。
...
\item \HIDX{ignore-case}{ＩＧＮＯＲＥ−ＣＡＳＥ}{H0.4.6.4.0.115}

ローマ字かな変換テーブルには普通小文字の規則だけが定義されており、通常
は大文字を入力するとアルファベットのまま入力され、かなには変換されませ
ん。
ignore-case を t にした場合、大文字で入力してもローマ字かな変換では小
文字として取り扱われます。
デフォルトは nil です。
...
\item \HIDX{index-hankaku}{ＩＮＤＥＸ−ＨＡＮＫＡＫＵ}{H0.4.6.4.0.116}
...
\item \HIDX{index-separator}{ＩＮＤＥＸ−ＨＡＮＫＡＫＵ}{H0.4.6.4.0.117}

候補一覧の番号を半角文字にカスタマイズしている場合の、番号と候補との区
切りのセパレータ文字を指定します。? の次の 1 文字をセパレータ文字とし
て使用します。 
デフォルトはピリオドです。
...
% ------- ・
\hptar{label=H0.4.6.4.0.117.5}
\item \HIDX{inhibit-hankaku-kana}{ＩＮＨＩＢＩＴ−ＨＡＮＫＡＫＵ−ＫＡＮＡ}{H0.4.6.4.0.117.5}

文字種変換で半角カタカナを禁止するどうかを指定します。t のときは禁止します。
デフォルトは nil です。
\begin{nquote}{2em}
\begin{verbatim}
(例)
    ;半角カタナナを禁止します。
    (setq inhibit-hankaku-kana t)
\end{verbatim}
\end{nquote}
...
ＯＦ−ＢＵＮＳＥＴＳＵ}{H0.4.6.4.0.118}

最右文節で次文節へ移動しようとしたときに、確定するか否かを指定します。
t で確定します。nil を指定すると最左文節がカレント文節になります。
デフォルトは nil です。また、stay-after-validate が nil のときで 
kakutei-if-end-bunsetsu が t のときは最右文節で候補一覧状態から候補を 
選択すると全文が確定します。
...
\item \HIDX{kouho-count}{ＫＯＵＨＯ−ＣＯＵＮＴ}{H0.4.6.4.0.119}
...
% ------- ・
\hptar{label=H0.4.6.4.0.119.5}
\item \HIDX{mojishu-continue}{ＭＯＪＩＳＨＵ−ＣＯＮＴＩＮＵＥ}{H0.4.6.4.0.119.5}

文字種変換後、文字種変換部分を確定しないで次の入力を行うかどうかを指
定します。t のときは確定しないで次の入力を行います。
デフォルト値は t です。

\begin{nquote}{2em}
\begin{verbatim}
(例)
    (setq mojishu-continue t)
\end{verbatim}
\end{nquote}
...
行われるようにすることができます。n-henkan-for-ichiran に対して数を指
定すると、 指定された回数変換キーを押すことにより候補一覧が表示されます。
...
\item \HIDX{n-kouho-bunsetsu}{Ｎ−ＫＯＵＨＯ−ＢＵＮＳＥＴＳＵ}{H0.4.6.4.0.122}

逐次自動変換時に漢字に自動的に変換された文節をいくつまで確定せずに保持
するかを 指定します。3 〜 32 の範囲で指定してください。
デフォルトは 16 です。
...
指定します。デフォルトは t です。nil を指定すると、候補一覧の番号が表
示されません。また、数字キーを押した場合、選択されている候補が確定し、
押したキーは次の読みの入力として取り扱われます。 
...
\item \HIDX{romaji-yuusen}{ＲＯＭＡＪＩ−ＹＵＵＳＥＮ}{H0.4.6.4.0.125}
...
{ＱＵＩＣＫＬＹ−ＥＳＣＡＰＥ−ＦＲＯＭ−ＫＩＧＯ−ＩＮＰＵＴ}{H0.4.6.4.0.126}
...
\item \HIDX{select-direct}{ＳＥＬＥＣＴ−ＤＩＲＥＣＴ}{H0.4.6.4.0.128}

numerical-key-select が t であるときに、数字キーで候補を選択したときに
候補一覧表示のままかそうでないかを指定します。
t の場合は候補一覧が終了します。デフォルトは t です。
...
\hpref{count=1,label=H0.4.7.0.0.3}2 & 候補を選択したら次の文節に移動したい。 \\
\hpref{count=1,label=H0.4.7.0.0.4}3 & 一太郎\footnotemark のような操作法で入力したい。 \\
\hpref{count=1,label=H0.4.7.0.0.5}4 & 「と［を両方使いたい。 \\
\hpref{count=1,label=H0.4.7.0.0.6}5 & 拡張メニューを自分で定義したい。 \\
\hline
\end{tabular}
...
% ------------------- (2)
% misao … V3.3 では、これをデフォルトにしたので、コメントアウトする。
%
...
メニューが表示され、記号入力、単語登録、変換方式の変更などが
